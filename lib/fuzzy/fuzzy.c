...
int fuzzy_hash_file(FILE *handle, /*@out@*/ char *result)
{
  ...
  if (fpos < 0)
    return -1;
  if (fseek_t(handle, 0, SEEK_END) < 0)
    return -1;
  fposend = ftell_t(handle);
  if (fposend < 0)
    return -1;
  if (fseek_t(handle, 0, SEEK_SET) < 0)
    return -1;
  if (NULL == (ctx = fuzzy_new()))
    return -1;
...
out:
  if (status == 0)
  {
    if (fseek_t(handle, fpos, SEEK_SET) < 0)
      return -1;
  }
  fuzzy_free(ctx);
  return status;
